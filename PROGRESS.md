...
`.gear-size-fields.is-expanded`（`data-gear-size-fields`，裝備勾選後就地展開的容器，本來是設計給「幾個尺寸輸入欄位」用的）設定了 `max-height: 600px` + `overflow: hidden`。2026-09-10 加入的裝備尺寸對照表被直接塞進同一個容器裡，雪服/雪服帽鏡組性別未選時女+男兩組全部顯示，實測真實內容高度（`scrollHeight`）達 **1525px**，遠超過 600px 的天花板——因為是 `overflow: hidden` 不是 `auto`，超出的部分整段消失、不會有任何捲軸提示。外套排在褲子前面，通常剛好落在 600px 以內所以看起來正常，褲子（尤其男款）排在更後面，經常直接被整段裁掉，這解釋了業主看到的「外套永遠正常、褲子永遠從某處開始消失、完全沒有橫向捲軸」——**捲軸「沒有」不是繪製時機問題，是這個維度的內容根本沒進入可視 DOM 範圍，瀏覽器判斷沒有東西需要橫向捲動，自然也不會畫捲軸。**

用 `getComputedStyle()` 從褲子表格的 `<table>` 逐層往上量測，確切定位到的唯一責任層（跟外套共用同一層）：`<div class="gear-size-fields is-expanded" data-gear-size-fields>`，修復前 `max-height` 實測 `598.894px`（對應原始碼設定值 `600px`）。往下比對相對位置：`.gear-size-fields` 頂端算起，女款外套佔 212~417px（完全落在天花板內，正常顯示），女款褲子佔 459~663px（**底部超出天花板 63px**，被裁掉）。外層還有 `.accordion-content.is-expanded { max-height: 3000px }`／`.accordion-card { overflow:hidden }` 等其他限制層，但實測都還有充裕空間（總內容 2396px < 3000px），不是這次裁切的成因。

### 精確修復（範圍收斂到單一 CSS 規則，沒有用業主那種往上暴力改整條祖先鏈的方式）

`assets/course-stage2-module.js` 的 `injectStylesOnce()`，唯一一行：
```css
/* 修復前 */ .gear-size-fields.is-expanded { max-height: 600px; ... }
/* 修復後 */ .gear-size-fields.is-expanded { max-height: 3000px; ... }
```
選 3000px 是跟外層 `.accordion-content.is-expanded` 的天花板同值（該值已在其他地方驗證過安全），不是憑感覺挑的數字；也確認過雪服帽鏡組女+男全展開的最大內容高度（1525px）遠低於這個新天花板，不會變成新的裁切點。**沒有動 `.cs2-legal-table` 這個 class 本身、也沒有動裝備租賃法律聲明賠償表所在的 `.cs2-legal-scrollbox`**（查證確認它是完全獨立、不在 `.gear-size-fields` 底下的容器，維持原本自己的 `max-height:200px; overflow-y:auto` 不受影響，8 列資料原封不動）。

### 修復後驗證（真正桌面瀏覽器，不加任何 console hack）

- `.gear-size-fields` 修復後 `scrollHeight`（1525）＝`clientHeight`（1525），**內容 100% 完整可見，不需要任何手動介入**。
- 女／男款・外套／褲子四張表 `tbody tr` 列數逐一核對：4／4／5／5，尺碼標籤（S/M/L/XL、M/L/XL/2XL/3XL）全部正確——**男款・褲子過去會被整段裁掉的 5 列，這次截圖確認完整顯示，包含各自獨立的橫向捲軸**。
- 外層 `.accordion-content.is-expanded` 沒有變成新瓶頸（2396px 內容 < 3000px 天花板）。
...
## ✅ 2026-09-10：Stage 3 加購表單新增「裝備尺寸對照表」

**背景**：業主要求在 Stage 3 加購表單（購物車頁裝備加租）幫客人選尺寸時提供參考對照表，這次只做 Stage3 這部分，獨立裝備租賃頁面留到下一輪；圖片素材業主之後才提供，這次先用文字註記佔位，不因為沒圖片卡住開發。

**涵蓋範圍**：`GEAR_ITEMS`（`assets/course-stage2-module.js`）裡有 `sizeFields` 的品項——安全帽、雪服、雪服帽鏡組、滑雪護具，共 4 項；單板鞋組（維持現有直接填數字的做法，不需要對照表）跟雪鏡（沒有 `sizeFields`）依業主指示不顯示對照表，已用 JS 直接檢查 DOM 確認這兩項確實沒有渲染出 `.cs2-size-reference`。

### 實作方式

- 沿用專案既有的「摘要＋可展開」模式，用原生 `<details>/<summary>`（比照 `snippets/cart-products.liquid` 已經在用的手法），沒有另外寫一套展開/收合 JS。
- 對照表本身重用既有的 `.cs2-legal-table`／`.cs2-legal-note` CSS class（裝備租賃風險聲明那個表格already在用的樣式），沒有重新設計一套新樣式，只新增了 `<details>` 外殼跟收合箭頭的 CSS。
- 渲染位置：緊接在該裝備的尺寸輸入欄位之後、還在同一個 `.gear-size-fields` 展開容器裡（`renderSizeReferenceDetailsHtml()`），符合「填尺寸欄位的地方旁邊」的要求；資料收集邏輯（`getSelectedGear()`）只讀 `[data-gear-size-field]` 這個選擇器，對照表的 HTML 完全不在這個範圍內，不會被誤讀成使用者填寫的資料。
- **雪服／雪服帽鏡組動態依性別只顯示對應那組（優先做的版本，不是退而求其次版）**：新增 `wireSizeReferenceGenderToggle()`，監聽該學員的共用性別欄位（`[data-attendee-gender-input]`），變動時只顯示 `data-gender-table` 屬性跟性別值相符的那組，未選性別時兩組都顯示（各自標「女款」/「男款」）。跟既有的 `wireDependentSizeFields()`（負責雪服尺碼下拉選單依性別灌選項）在同一輪迴圈裡一起呼叫，行為互不干擾。
- 雪服帽鏡組（combo 品項）的對照表同時包含雪服/帽鏡尺碼對照＋安全帽頭圍對照兩張表，用「安全帽頭圍對照」小標題分隔。

### 資料內容

| 裝備 | 對照表內容 | 資料狀態 |
|---|---|---|
| 安全帽 | S(52-55cm)／M(55-59cm)／L(59-63cm)，頭圍 | 完整（沿用 `HELMET_SIZE_OPTIONS` 旁已確認過的數字） |
| 滑雪護具 | 護臀 S(56-66)/M(60-74)/L(70-80)/XL(74-88)cm 腰圍；護膝 S對應32-38cm、M/L/XL共用38-52cm | 完整（護臀沿用 `PAD_VARIANT_ID_BY_SIZE` 旁已確認過的數字，護膝是這次任務新提供的資料） |
| 雪服／雪服帽鏡組 | 女款 S/M/L/XL、男款 M/L/XL/2XL/3XL，各自衣長/袖長/胸圍/肩幅/身高/體重六個維度 | 初版是佔位符「－」，**同一天稍後業主已補提供實際數字，見下方「補件：雪服/雪服帽鏡組實際數字」小節** |

### 驗證結果

- **桌機截圖**：安全帽、雪服（含女款/男款兩組表格）、雪服帽鏡組（雪服尺碼表＋安全帽頭圍表）、滑雪護具，各裝備展開狀態逐一截圖確認文字內容跟排版正常，`<details>` 展開箭頭（▸/▾）正常運作。
- **性別動態篩選驗證**：學員性別從未選（兩組都顯示）切換成「女」後，用 DOM 檢查＋截圖雙重確認「男款」表格正確被隱藏（`cs2-gender-table-hidden` class），「女款」表格維持顯示；套用到雪服帽鏡組的組合表格也正確同步。
- **手機 375px**：初版截圖發現雪服/雪服帽鏡組的 7 欄表格（尺寸+6維度）在 375px 寬度下欄位被壓到文字換行、可讀性不佳——**這是這次驗證主動抓到並修正的問題，不是業主要求清單裡的項目**，修法是幫這個 7 欄表格額外包一層 `overflow-x:auto` 讓表格用自己的寬度橫向捲動（`.cs2-size-reference-table-scroll`，`min-width: 420px`），沒有動共用的 `.cs2-legal-table` 本身（避免影響它原本 2~3 欄的其他用途，例如租賃風險聲明表格）。修正後截圖＋實際橫向捲動操作確認欄位標題單行顯示、捲動後可以看到「肩幅/身高/體重」欄位，不再擠壓變形。安全帽（2欄）、滑雪護具（3欄）本來就沒有這個問題，不需要加捲動容器。
- **簡單送出流程驗證**（業主表示新增顯示內容不影響既有送出邏輯，可自行判斷要不要驗證，這次做了一輪）：勾選雪服帽鏡組、展開對照表、切換性別、填尺寸、送出 → Stage3 確認彈窗（Checkpoint 2 新增的功能）正確顯示，對照表內容沒有洩漏進彈窗摘要或 `/cart.js` 的 properties 裡（只有真實填寫的尺寸值）→ 按確認 → `/cart.js` 最終結果核對正確，`_stage2_completed` 正常寫入。測試完成已清空購物車。

**沒有發現任何跟既有邏輯衝突的問題。**

### 補件：雪服/雪服帽鏡組實際數字（同一天稍後）

業主提供了雪服/雪服帽鏡組完整的實際尺寸數字，取代上面的佔位符「－」。

**查證發現跟原始規格的結構落差**：原始規格描述是「單一 6 欄表格（衣長/袖長/胸圍/肩幅/身高/體重）」，但業主提供的實際數字是**「外套」「褲子」兩份各自獨立的量測表**（外套：著丈/袖丈/胸圍/肩幅/身高(cm)/體重(kg)；褲子：腰圍/臀圍/大腿圍/股上(前/後)/褲長/褲口）——雪服本身就是外套+褲子一套，不是單一件。已照實際資料結構拆成兩張表（`CLOTHING_JACKET_DATA`／`CLOTHING_PANTS_DATA`，`assets/course-stage2-module.js`），尺碼列（S/M/L/XL、M/L/XL/2XL/3XL）維持跟 `CLOTHING_SIZE_BY_GENDER` 一致沒有調整，只是欄位結構從 1 張表變成 2 張表。

**驗證結果**：
- 桌機截圖確認女款/男款各自的外套表＋褲子表數字逐項核對跟業主提供的資料一致（例如女款外套 S：著丈72/袖丈57/胸圍120/肩幅54/身高155-165/體重45-50，男款褲子 3XL：腰圍95-103/臀圍124/大腿圍76/股上35-48/褲長105/褲口52，皆正確）。
- 用 DOM 查詢確認雪服帽鏡組的組合對照表裡 5 張表格（女外套/女褲子/男外套/男褲子/安全帽）全部都在，逐一截圖／`scrollIntoView` 確認內容完整，先前截圖一度因為滾動幅度估計不準看似「男款不見了」，用 DOM 直接查證排除是真的資料遺漏，純粹是截圖操作問題。
- 手機 375px 重新驗證 Checkpoint 1 修好的橫向捲動容器（`.cs2-size-reference-table-scroll`）在真實數字（含較長字串如「155-165」「32/45」）下依然正常運作，欄位標題維持單行不換行，每張表各自獨立橫向捲動，捲動後可以看到「肩幅/身高(cm)/體重(kg)」等後段欄位。
- 沒有重跑完整送出流程驗證（業主表示不需要，這次純粹是資料填值，不影響邏輯）。

**這次任務只剩「量測方式圖示」等業主提供正式素材後才能做，其餘文字/數字內容已全部到位。**

### 補件後第二輪業主回報「女款・褲子缺 XL」——查證結論：資料/渲染完整，純視覺框取問題（沒有動程式碼）

業主再次截圖反映「雪服帽鏡組」底下的「女款・褲子」表格只看到 S/M/L，捲到底直接接到下一張裝備卡片（安全帽），沒有 XL。業主同時指出一個查證盲點：**上一輪查證只驗證了「雪服帽鏡組」這個組合品項，沒有分別驗證「雪服」這個獨立品項**——兩者各自獨立呼叫同一份 `clothingSizeReferenceHtml()`，理論上輸出應該一致，但沒有分別做過 DOM 查證就不能算「兩者都驗證過」。

**這次重新查證，兩個品項分開、各自明確標註**：

- **「雪服」（獨立品項，`data-gear-key="雪服"`）**：DOM 查詢＋`data-gear-key` 身份核對，「女款・褲子」表格 4 列（S/M/L/XL）全部存在，XL 列數值 `79-87/116/72/34/47/103/50` 逐字元核對跟業主資料一致。**實際截圖重現了業主回報的畫面**——捲到 L 列時視窗確實剛好卡在 L 列下緣，下方緊接著「安全帽 +$300」卡片，跟業主截圖一模一樣；用 `scrollIntoView` 把 XL 列帶進可視範圍後，XL 列清楚可見，證實只是還沒捲完，不是內容不存在或被截斷。
- **「雪服帽鏡組」（組合品項，`data-gear-key="雪服帽鏡組"`）**：同樣方式獨立驗證，「女款・褲子」4 列同樣完整存在，數值一致。

**結論：兩個品項的對照表資料/渲染都完整正確，沒有發現任何程式碼層級的缺漏（迴圈邊界、資料源頭都沒有問題），這次沒有修改任何程式碼。** 已把這個「內容夠長容易被誤判成截斷」的視覺特性記錄進本文件最上方「接續指引」，供之後接手/業主回報類似狀況時先參考，不用每次都重新排查一輪。
...
## ✅⚠️ 2026-09-07（晚間）：三項視覺修正——Hero捲動圖示、麵包屑文字、全站標題風格統一

**背景**：Eric 在 `/collections/all` 發現三個問題：Hero banner 上出現不該有的捲動提示圖示、麵包屑「商品」文字想改成「所有商品」、全站各頁面標題風格（字級/顏色）不一致。

### 1. Hero 捲動提示圖示（SCROLL icon）——已修正並驗證

**根因**：`sections/hero.liquid` 這個共用 section 檔案裡，捲動提示的 HTML 完全沒有條件判斷，寫死一定會渲染——只要頁面用到「hero」這個 section 類型就會出現，不是只有首頁才有。這段程式碼從專案最初的 Initial commit 就存在（`git log -S` 確認），不是這次任務或之前任何一次修改造成的。

**修法**：加上 `{% if template.name == 'index' %}` 判斷，只在首頁顯示。已推送並驗證：`/collections/all`、`/collections/全日-半日-滑雪課程` 都確認不再出現 SCROLL 圖示；首頁重新確認 `.scroll-down-indicator` 仍然存在，沒有被誤刪。

### 2. 麵包屑「商品」→「所有商品」——已修正並驗證

**根因**：`snippets/breadcrumbs.liquid` 的 collection 分支用 `{{ collection.title }}` 顯示麵包屑文字，這個全域 `collection` 物件跟區塊層級的 `closest.collection` 物件，對 Shopify 自動產生的 `all` 這個系統 collection 會解析出不一致的值（詳見下方第 3 點的延伸發現）。

**修法**：針對 `collection.handle == 'all'` 的情況，麵包屑直接顯示固定文字「所有商品」，不影響其他 collection 的麵包屑（其他 collection 兩邊資料本來就一致，不受影響）。已驗證：`/collections/all` 麵包屑正確顯示「所有商品」，「全日/半日滑雪課程」等其他 collection 麵包屑維持原本正確的各自標題。

### 3. 全站頁面標題風格統一（H2、海軍藍 `#1A2E4A`）——已修正並驗證，含一個已知平台層級的殘留問題

**查證發現的真正原因**：不是單純的顏色設定不一致，而是**多數頁面都有「Banner 區塊自己的標題」+「主要內容區塊自己的標題」兩份重複標題疊在一起**（跟 9/5 那次修過的 `equipment-rental` Banner 重複標題是同一種問題，這次發現同樣的問題也存在於「常見問題」「聯絡我們」頁）。另外實測發現一個關鍵細節：**視覺大小主要是由標題文字實際使用的 HTML 標籤（`<h1>`／`<h2>`／`<p>`）決定，不是只看 `type_preset` 這個下拉選單設定**——同樣設定 `type_preset: h2`，字面標籤是 `<h1>` 就會渲染成 48px/700，字面標籤如果是 `<p>` 反而完全吃不到標題樣式（只有 16px/400）；只有字面標籤也是 `<h2>` 時，才會正確套用 30px/600 的 H2 樣式。這個判斷過程中曾經先誤改成 `<p>` 標籤（一度導致「常見問題」標題整個縮成 16px 小字），發現後已改回 `<h2>` 標籤並重新驗證通過，過程記錄下來避免下次重蹈覆轍。

**修正內容**：
- `裝備租賃`／`常見問題`／`聯絡我們` 三個頁面：移除 Banner 區塊自己的重複標題（沿用 9/5 equipment-rental 的清理手法：拿掉標題/佔位文字/按鈕三個 block，保留背景圖），只保留主要內容區塊的標題，統一改成 `<h2>` 標籤、`type_preset: h2`、顏色明確寫死 `#1A2E4A`
- `templates/collection.json`／`collection.all.json`：標題顏色也明確寫死 `#1A2E4A`（不再依賴 color scheme 間接繼承）

**驗證結果（直接量測 computed style，非目測）**：

| 頁面 | 標籤 | 顏色 | 字級 | 字重 |
|---|---|---|---|---|
| 裝備租賃 | H2 | rgb(26,46,74) | 30px | 600 |
| 常見問題 | H2 | rgb(26,46,74) | 30px | 600 |
| 聯絡我們 | H2 | rgb(26,46,74) | 30px | 600 |
| collections/all | H2 | rgb(26,46,74) | 30px | 600 |

四個頁面標題現在字級/字重/顏色完全一致。桌機、手機 375px 都截圖確認排版正常，Banner 清空後沒有留下突兀空白。

**「商品」vs「所有商品」殘留問題——已由 Eric 解決 ✅**：修正過程中發現 `closest.collection.title`（區塊層級）對 Shopify 自動產生的 `all` collection 會不穩定地解析出「商品」或「所有商品」兩種不同結果，即使把文字寫死成「所有商品」並確認推送到遠端檔案正確，線上實際渲染一度仍顯示「商品」，判斷是 Shopify 平台層級的內容快取問題，theme 檔案推送無法強制更新。**Eric 透過 Shopify 後台的 Sidekick 協助處理後，已確認正確顯示「所有商品」**，重新查證：文字正確、樣式維持一致（`rgb(26,46,74)` 海軍藍、~30px、字重 600，跟裝備租賃/常見問題/聯絡我們三個頁面完全一致，沒有因為這次修正跑掉）。**這三項視覺修正到此全部確認完成，沒有殘留問題。**

---

## ✅✅ 2026-09-07：「View all / collections/all」整理工作全部結案

**這輪處理的完整範圍**：星野商品封存（上一輪）之後，接續處理「View all」點進去的 `/collections/all` 頁面整理，包含：(1) 封存 7 個不再需要的舊環境課程/測試裝備商品、(2) 建立 `/collections/all` 專屬模板、(3) 評估商品分組視覺呈現方案。三項都已完成查證/驗證，**Eric 最終決定分組視覺優化（第 3 項）暫緩不做**，維持目前 11 個乾淨商品的單一清單即可——這是這次「View all / collections/all」相關工作的最終結果，正式結案。

### 專屬模板 `templates/collection.all.json` 最終確認

除了上一輪的功能驗證外，這次額外直接 `shopify theme pull` 把正式站遠端主題的 `templates/collection.all.json` 抓下來核對（不是只看本機檔案），確認檔案（18,258 bytes）確實存在於正式站遠端主題（Designer_Eric #147355926611），內容跟本機一致。**確認 Eric 之後可以用 `https://qgfchv-py.myshopify.com/admin/themes/147355926611/editor?previewPath=/collections/all` 進主題編輯器，獨立編輯這一頁的 Hero banner，不會影響其他 collection 頁面**。另外重新查了一次 `/collections/all/products.json`，確認商品數維持 11 個、乾淨清單沒有變化。

### 封存驗證（Eric 已在後台操作，這次負責驗證）

Eric 已封存 7 個商品：4 個無星號舊課程（`fullday-class-peak-season`／`fullday-class-off-season`／`halfday-class-peak-season`／`halfday-class-off-season`）＋ 3 個帶星號測試裝備（`測試專用-裝備租賃-單板鞋組`／`test-gear-rental-all`／`test-gear-rent-helmet`）。

驗證結果全部通過：
- `/collections/all/products.json`：商品數 18 → **11**，7 個封存商品全部消失，剩下清單乾淨（4 星號課程＋6 真實裝備＋教練加購）
- 7 個舊網址全部回傳 404，實際導航確認呈現正常 404 頁面
- **真實流程重點驗證**：`*全天滑雪課程(淡季優惠)` ＋ 富良野 ＋ 勾選單板鞋組/安全帽/雪鏡三項裝備，送出後 `/cart.js` 確認裝備都是**沒有星號的真實商品**正確計費（雪鏡$300、安全帽M號$300、單板鞋組$1,200，課程$11,700，總額$13,500 全部正確），properties（身高/體重/鞋子尺寸/性別/雪場）也正確寫入——**確認封存這 3 個星號測試裝備完全沒有影響到真實加購計費流程**。測試完成已清空購物車。

### 任務二：查證 tag 現況，發現原生排序無法依標籤分組，Eric 最終決定暫緩不做

查證 `/collections/all` 11 個商品的 tag：4 個課程統一有 `test-course`、6 個裝備統一有 `gear-rental`，**tag 本身乾淨、足夠識別分組**。

但**發現 Shopify 原生 collection 排序設定沒有「依標籤排序」這個選項**（可選項只有 best-selling/字母/價格/日期/手動），上次評估時判斷有誤，這次已更正。而且「在商品清單中間插入純文字分隔標題」這件事，原生 `main-collection` 區塊本身的架構也不支援（不管用什麼排序都一樣做不到），要做到的話需要另外寫一個專屬 section 檔案，工作量比原本評估的高。

**發現一個零成本的替代方案**：課程商品價格全部 ≥$7,650、裝備商品全部 ≤$1,200，價格區間完全不重疊，**把 collection 排序設成「價格高到低」就能讓兩組商品自然分開**，不需要任何程式碼或後台 tag 操作，但沒有文字分隔標題。已提供這個選項（選項 D）跟「另寫專屬 section 做出真正文字分隔線」（選項 E，工作量較高）給 Eric 選擇。

**Eric 最終決定（2026-09-07）：兩個選項都先不採用，維持目前無分組的乾淨清單**。理由：導覽選單已有明確分類入口，這頁定位是給隨便逛逛的客人看，不分組不影響核心體驗；選項 D 依賴「課程一定比裝備貴」的價格假設，未來新增商品可能失效；選項 E 工作量較高，兩者都留到之後有餘力再回顧優化，已記錄進文件下方「待辦事項」第 16 項，方便之後直接參考不用重新查證。**這次沒有做任何排序或分組調整，`/collections/all` 目前狀態（11 個乾淨商品、無分組）就是最終結果。**

---

## 🔧 2026-09-07：`/collections/all` 拆出專屬模板 `collection.all.json`

**背景**：Eric 確認要讓 `/collections/all` 有自己專屬、不影響其他 collection 的 Hero banner，且已經知道之前查證過「共用 `collection.json` 導致無法獨立編輯」這件事。

**異動**：`templates/collection.all.json` 已建立（複製當時 `collection.json` 的完整內容，含 9/5 那次視覺調整的卡片樣式），已推送到正式站。Shopify 會自動把 handle 是 `all` 的 collection 路由到這份專屬模板，其餘 collection（全日/半日滑雪課程、北海道富良野全天/半天、equipment-rental）繼續共用原本的 `collection.json`，兩邊從此互不影響。

**驗證**：`/collections/all` 沿用先前的卡片樣式正常顯示（18 個商品、卡片底色 `rgb(232,244,250)`、CTA 按鈕都在）；另外查了「全日/半日滑雪課程」collection 確認還是走共用模板、4 個商品正常顯示，沒有因為新增這份專屬模板而受影響。**Eric 現在可以用 `https://qgfchv-py.myshopify.com/admin/themes/147355926611/editor?previewPath=/collections/all` 進到主題編輯器，編輯這一頁的 Hero 只會影響 `/collections/all` 自己，不會動到其他 collection。**

「View all」實際內容要怎麼呈現（分類分組方式）尚待 Eric 選定方向，這次只先把模板拆開，內容本身還沒有調整。

---

## ✅ 2026-09-05：兩項視覺調整——裝備租賃頁 Banner 清理、`/collections/all` 卡片樣式比照裝備租賃頁

### 任務一：`/pages/equipment-rental` 清理 Banner 跟重複標題

**異動（`templates/page.equipment-rental.json`）**：
- Banner 區塊（`section_RTQaqd`）移除全部 3 個內容 block（標題「裝備租賃 EQUIPMENT RENTAL」、Horizon 預設佔位文字「精心製作，深受顧客喜愛。」、「立即選購」按鈕），保留區塊本身（背景圖＋色系＋高度設定），變成純視覺照片橫幅，不再疊文字
- 移除 `er_cards_xK9mLp`（custom-liquid 卡片格線）內部寫死的 `.er-section-title`「裝備租賃 Equipment Rental」子標題 div，以及對應的孤兒 CSS 規則，避免跟 `main-page` 的 `heading` block（頁面唯一保留的正式標題）重複
- 頁面標題現在只出現一次（麵包屑之後），Banner 純照片無文字，兩者之間、標題跟卡片格線之間都用既有的區塊間距自然銜接，沒有出現突兀留白

**驗證**：正式站桌機、手機 375px 都確認過，Banner 顯示為乾淨照片、標題只出現一次、卡片格線緊接在標題下方無異常空白，其餘裝備卡片內容（圖片/說明/價格/按鈕）完全沒受影響。

### 任務二：`/collections/all` 原生商品卡片樣式比照裝備租賃頁

**技術現況（先查證後動手）**：`main-collection` 這個原生 section 本身**沒有 blocks schema**（無法直接插入 custom-liquid 區塊），且全站只有一份共用的 `templates/collection.json`（沒有針對 `all` 另外做模板），所以：
- CSS／字型／CTA 按鈕改用「在 heading 區塊（`section` key）裡新增一個 `custom-liquid` block」的方式注入，`<style>` 規則用 `main[data-template="collection"]` 限定範圍——**只作用在 collection 模板頁面**，不會外溢到商品頁推薦區塊、購物車頁「You may also like」等其他地方共用的同一批 `.product-card`／`.card-gallery` class
- ⚠️ **範圍提醒**：因為全站只有一份 `collection.json`，這次樣式改動理論上會套用到所有走這個模板的 collection 頁（例如「全日/半日 滑雪課程」「北海道富良野...」等），不是只有 `/collections/all`——但這些其他 collection 目前都是完全預設樣式（沒有任何自訂），套用這次更好看的卡片樣式屬於視覺一致性提升，沒有發現任何負面影響；如果之後要讓 `/collections/all` 跟其他 collection 樣式不同，需要另外建一份 `templates/collection.all.json` 才能做到，這次沒有做這一步（不在這次任務範圍內）
- CTA 按鈕（`_product-card-group` 支援 `custom-liquid` 子 block）用 `{{ closest.product.url }}` 動態產生連結，逐一驗證過全部 18 個商品的按鈕都正確連到各自商品頁
- 圖片比例原生選項沒有 4:3，改用 CSS 直接覆寫 `--gallery-aspect-ratio`／`aspect-ratio`

**四個差異點逐項比對結果（正式站直接量測 computed style，不是憑印象）**：

| 項目 | 目標值 | 正式站量測結果 |
|---|---|---|
| 卡片背景/邊框 | `#E8F4FA` 底／`#B8D9ED` 邊框 | `rgb(232,244,250)` / `1px solid rgb(184,217,237)` ✓ |
| 卡片圓角 | 16px | `16px` ✓ |
| 圖片比例 | 4:3 | `aspect-ratio: 4 / 3` ✓ |
| 標題字級/字重/顏色 | 17px / 600 / `#111111` | `17px` / `600` / `rgb(17,17,17)` ✓ |
| 價格字級/字重/顏色 | 22px / 700 / `#1A2E4A` | `22px` / `700` / `rgb(26,46,74)` ✓ |
| 字型 | Inter + Noto Sans TC | `document.fonts.check('700 22px Inter')` → `true`；`'700 17px "Noto Sans TC"'` → `true`，確認字型檔真的載入生效，不是只有宣告 |
| CTA 按鈕 | 深藍實心「立即預訂 →」 | 存在，`background: rgb(26,46,74)`，href 逐一核對全部 18 個商品皆正確 ✓ |

**功能不受影響（逐項確認）**：
- 星野排除結果不受影響：頁面上 18 個商品清單跟改動前完全一致，沒有星野
- 原生篩選/排序 UI（可用性、價格區間，價格上限正確顯示 $16,200）完整存在、正常運作
- 無限捲動分頁沒有被動到（沒有改動 `main-collection.liquid`／`results-list` 任何邏輯，純粹加 CSS＋新增的 CTA 按鈕 block）
- 桌機、手機 375px 都截圖確認過排版正常，2 欄手機格線無擠壓變形

**⚠️ 截圖交付限制**：這次沒辦法附上修改前後的截圖檔案——瀏覽器工具的截圖只能顯示給我自己確認，沒有辦法存成檔案傳給 Eric。改成用上表列出的實際量測數值（直接讀 computed style，不是憑印象）逐項證明每個目標都達成，Eric 也可以直接到正式站 `/collections/all`、`/pages/equipment-rental` 桌機/手機實際查看（改動已經上線）。

---

## ✅ 2026-09-05：購物車頁「You may also like → View all」跑出星野舊商品——已排除，議題正式結案

**背景**：業主反映購物車頁「You may also like」點「View all」會跑出星野系列舊官網遺留商品。

**查證結果**：「View all」連到 `https://lifechillsnow.com/collections/all`——Shopify 自動產生的「全部商品」collection，**沒有手動排除機制，只要商品還是上架狀態就一定會被收錄**，跟商品 tag／collection 設定無關。星野 4 個商品當時仍是上架中狀態，所以被自動收錄。商品頁本身的「You may also like」是 Shopify 原生 `related` 推薦演算法（依 tag/type 比對），實測當下沒有推薦出星野商品，且課程商品頁這個區塊本來就被 `course-booking-form.liquid` 的 CSS 規則整個隱藏。

**業主決策**：跟業主確認星野系列已永久停用，採用方案 C（治本）——直接把 4 個星野商品在 Shopify 後台改成 **Archived（封存）**狀態（業主親自操作，非刪除，保留商品資料）。

**最終驗證（全部通過）**：
1. **`/collections/all/products.json`**：直接查證（用全新分頁、確認 origin 是 `https://lifechillsnow.com` 才讀取，避免瀏覽器分頁污染疑慮）——商品數從原本 22 個變成 **18 個，星野 4 個商品完全不在清單裡**。
2. **舊網址訪問狀態**：4 個星野商品舊網址（`fullday-class-peak-season-hoshino` 等）直接 fetch 全部回傳 **404**，實際導航確認呈現主題既有的正常 404 頁面（標題「404 找不到」），不是破損的購買頁面。
3. **其他商品不受影響**：`/products.json` 確認其餘 18 個商品（4 個星號課程、9 個裝備租賃、4 個無星號舊環境課程、教練加購）全部正常在架；實際用 `/cart/add.js` 分別加入一個星號課程商品（\*全天滑雪課程(旺季)）跟一個真實裝備商品（裝備租賃 - 雪鏡），兩者都正確加入購物車、金額正確，購買流程未受影響。測試完成已清空購物車。

**額外備註**：4 個無星號的舊環境課程商品（widget 111783 那組）這次維持現狀，不在處理範圍內，跟業主討論過的一致。

**結論：星野系列商品已確認從所有客人看得到的地方完全移除（`collections/all`、舊網址皆已下架），其他商品購買流程未受任何影響。這個議題正式結案。**

---

## ✅ 2026-09-05：單板鞋組身高/體重/鞋子尺寸新增數值範圍防呆

**背景**：業主反映單板鞋組加購的身高/體重/鞋子尺寸三個欄位沒有防呆，隨便填數字都能送出。

**第一步查證**：確認業主反映屬實——欄位是 `type="number"` 但只有 `min="0"`、沒有 `max`（[course-stage2-module.js:354-359](assets/course-stage2-module.js:354)）；送出時的 JS 驗證 `findAttendeesWithIncompleteGearSizes()` 只檢查欄位是不是空字串，完全沒檢查數值合不合理，任何非空數字（含極端值、負數字串）都會被當成「已完整填寫」放行。欄位定義 `unit: 'cm'` 確認鞋子尺寸是腳長 cm 量測，業主建議的 15~35cm 區間跟這個單位定義一致。

**第二步修復**：
- `GEAR_ITEMS` 的 height/weight/shoeSize 三個欄位新增 `min`/`max`：身高 100~220、體重 20~150、鞋子尺寸 15~35
- `renderGearSizeFieldHtml()` 依欄位定義輸出 HTML5 原生 `min`/`max` 屬性（第一層防呆）
- 原本只檢查「空值」的 `findAttendeesWithIncompleteGearSizes()` 擴充重寫成 `findGearSizeIssues()`：不只依賴 HTML5 原生限制，送出前額外用 JS 明確比對數值範圍（含非法數字 `isNaN`），超出範圍/非數字一律擋下送出，並清楚顯示是哪位學員哪個欄位不合理（例如「學員1「身高」需介於 100~220cm」），跟「尺寸資訊未完成」分開訊息，客人才分得清是沒填還是填錯
- 已推送到正式站（Designer_Eric #147355926611）

**驗證（真實流程，全部通過）**：

| 測試項目 | 結果 |
|---|---|
| 身高邊界：99 / 100 / 220 / 221 | ❌擋 / ✅通過 / ✅通過 / ❌擋——邊界值精確 |
| 體重邊界：19 / 20 / 150 / 151 | ❌擋 / ✅通過 / ✅通過 / ❌擋 |
| 鞋子尺寸邊界：14 / 15 / 35 / 36 | ❌擋 / ✅通過 / ✅通過 / ❌擋 |
| 負數（身高 -50） | ❌正確擋下 |
| 空白 | ❌正確擋下，訊息「尺寸資訊未完成」 |
| 非數字（"abc"） | ❌正確擋下——`type="number"` 原生輸入框本身就拒絕輸入非數字字元，加上 JS 驗證雙重防護 |
| 三欄位同時超出範圍 | ❌正確全部擋下，三則訊息用「；」分開清楚列出 |
| 合理數值（175/68/27 等） | ✅正確送出，`/cart.js` 查證 properties 正確寫入 |

測試環境（星號商品 `test-course-halfday-offpeak`）跟正式環境（舊商品 `halfday-class-off-season`，widget 111783）都測過，行為一致；桌機跟手機 375px 都驗證過（手機版截圖確認排版正常、輸入框不擠壓變形）。每次測試完都已清空購物車。**單板鞋組數值防呆修復完成並驗證通過。**

---

## ✅ 2026-09-05：Stage3 指定教練新增第四位教練 Una

**背景**：業主反映指定教練加購只有阿哲/Angus/Kris 三個選項，但 Una 其實已經是 9/3 設定教練容量共享（BTA Resource 共用容量）時的 4 位教練資源之一（阿哲、Angus、Kris、Una），Stage3 加購清單沒有同步更新過。

**第一步查證**：`assets/course-stage2-module.js` 的 `COACH_ITEMS`（[course-stage2-module.js:153](assets/course-stage2-module.js:153)）確認就是一個純文字清單 `[{key,label}, ...]`，`renderCoachSelectSection()` 直接 loop 這個陣列產生單選清單；計費用固定 `COACH_PRICE = 400` 跟單一 `COACH_VARIANT_ID`（一顆 variant，跟選哪位教練完全無關，教練姓名純文字寫入 `指定教練` property）。**確認新增 Una 純粹是陣列多加一筆，不涉及任何新商品/variant**，判斷跟業主背景說明一致。

**第二步修復**：`COACH_ITEMS` 新增 `{ key: 'Una', label: '教練：Una' }`，已推送到正式站（Designer_Eric #147355926611）。

**驗證（4 組真實流程，全部通過）**：

| 測試 | 商品/環境 | 裝置 | 選擇教練 | 結果 |
|---|---|---|---|---|
| 1 | \*全天(旺季)（星號／widget 124456，網站實際在用） | 桌機 | Una | ✅ `指定教練: "Una"`，教練費 $400，1 筆 line item |
| 2 | 全天滑雪課程（無星號／widget 111783，舊環境） | 桌機 | Una | ✅ 同上，兩邊環境都正確 |
| 3 | \*全天(旺季) | 手機 375px | Una | ✅ 同上，4 位教練選項排版正常、無溢出（已截圖確認） |
| 4（回歸測試） | \*半天(旺季) | 桌機 | Kris（原本既有教練） | ✅ `指定教練: "Kris"`，教練費 $400，確認新增 Una 沒有影響原本三位教練的功能 |

每次測試都直接查 `/cart.js` 確認 property 跟金額，測試完都已清空購物車。**Una 教練選項新增完成並驗證通過。**

---

## 🚨✅ 2026-09-04（凌晨）：指定教練費重複計費——根因查明並修復，4 商品×多人數組合全部驗證通過

**背景**：Eric 回報 2 人課程指定教練會被加收兩筆 $400（共 $800），但指定教練費應該是綁課程本身、不是算人頭，一人跟四人應該都只收一次。

### 根因

`assets/course-stage2-module.js` 的 `addRealGearLineItems()` 成功呼叫 `/cart/add.js` 後會 dispatch 一個 `CartUpdateEvent`（事件名稱其實就是字串 `'cart:update'`，定義在 `assets/events.js` 的 `ThemeEvents.cartUpdate`）。`snippets/cart-stage2-trigger.liquid` 有 `document.addEventListener('cart:update', checkAndTrigger)`，會接住這個事件重新檢查購物車、判斷要不要彈出 Stage3 Modal。

2026-09-01 稍早那次「把加購/寫入改成依序執行」的修復（先呼叫 `addRealGearLineItems()`，成功後才呼叫 `writeCourseFormDataToCart()` 寫入 `_stage2_completed` 標記），把時序變成固定：**`addRealGearLineItems()` 完成、dispatch `cart:update` 的當下，`_stage2_completed` 必然還沒被寫入**。這代表這個中途觸發的 `cart:update` 事件，會在標記完成「之前」搶先跑一次 `checkAndTrigger()`，對同一筆還在送出中的課程項目重新彈出 Modal——如果這時候又送出一次（真實客人或測試時很自然會這樣做，因為看起來像是沒送出成功），教練費就真的被加購兩次。**這不是機率性 bug，是 9/1 那次修復必然會造成的副作用**，也解釋了為什麼是「今天」才出現（9/1 那次修復上線後才開始必然發生）。

### 修法

在 `cart-stage2-trigger.liquid` 加一個 `pendingSubmissionKeys` 鎖：`onSubmit` 一開始（呼叫 `addRealGearLineItems()` 之前）先把該 line item 的 key 標記為「送出中」，`findItemNeedingStage2()` 排除掉標記中的 key，讓中途觸發的 `checkAndTrigger()` 不會對同一筆還在處理中的項目重新彈出 Modal。成功或失敗都會解鎖（失敗要解鎖，維持原本「失敗後客人能重新送出重試」的設計，不能讓鎖卡死重試路徑）。已推送到正式站（Designer_Eric #147355926611）。

### 驗證：4 個商品 × 不同人數方案，全部真實走一次完整 BTA 流程 + 指定教練，確認金額

| 商品 | Stage1 人數方案 | 實際參加人數 | 教練 | 購物車教練費 line item 數 | 教練費金額 | 課程+教練費總額 |
|---|---|---|---|---|---|---|
| \*全天(旺季) | 3~4人 | 4人 | 阿哲 | 1 筆 | $400 | $16,600（$16,200+$400）✓ |
| \*全天(淡季優惠) | 3~4人 | 3人 | Angus | 1 筆 | $400 | $14,350（$13,950+$400）✓ |
| \*半天(旺季) | 1~2人 | 2人 | Kris | 1 筆 | $400 | $10,300（$9,900+$400）✓ |
| \*半天(淡季優惠) | 3~4人 | 4人 | 阿哲 | 1 筆 | $400 | $10,300（$9,900+$400）✓ |

四個商品、涵蓋兩種 Stage1 人數方案（1~2人／3~4人）跟三種實際參加人數（2/3/4人），**每一次都直接查 `/cart.js` 確認教練費 line item 只有 1 筆、quantity 恆為 1、金額恆為 $400**，完全不隨人數增加而變多，證實指定教練費現在正確地綁在「課」這個單位，不是算人頭。每次測試完都已清空購物車，沒有留下測試資料。

**結論：指定教練費重複計費問題已修復並驗證通過。**

---

## 🚨🔍 2026-09-03（晚間，緊急查證中）：富良野 Stage3 裝備/住宿加購卡片消失——根因已查明，**不是今天 ea85d1a 這批修復造成的回歸**，是 BTA 後台欄位標籤被改名

**背景**：Eric 回報測試富良野雪場，走到 Stage 3 卻沒看到裝備加租/住宿加購卡片，懷疑是今天 commit `ea85d1a`（淡旺季驗證、教練/裝備競態修正等）連帶造成的回歸。要求立即查證根因，不要先動手修。

### 排查方法

1. **先查 `ea85d1a` 這個 commit 的 diff**：`git show ea85d1a -- snippets/cart-stage2-trigger.liquid assets/course-stage2-module.js`，搜尋 `furano`／`resort-hidden`／`滑雪場` 關鍵字，**完全沒有命中**——今天這批 commit 一行都沒有動到雪場判斷/裝備顯示這段邏輯。
2. **查 `isFuranoItem()` 這個函式最後一次被改是哪個 commit**：`git log -S"RESORT_KEY_PROD"` 只有一筆命中，是 **2026-08-29 的 `944732a`**（順序6方向性修正那次），從那之後這段程式碼從未再被改過。
3. **走一次真實完整流程**（不是憑猜測）：`test-course-fullday-peak`，Stage1 選 12/20/2026，BTA 自己的 Stage2「填寫詳細資料」表單裡，「雪場區域」下拉選單選「富良野」，其餘必填欄位全部填完，送出 → 整頁導向 `/cart`。
4. **直接查真正寫進 `/cart.js` 的 line item properties**（不是看表單原始碼猜）。

### 根因：BTA 後台的 Booking Field 標籤跟選項，在 2026-08-29 之後被改過，現在的 key 已經不是程式碼在找的那個字串

`/cart.js` 查到的真實 property key 是 **`"雪場區域": "富良野"`**——不是程式碼裡 `RESORT_KEY_PROD = '滑雪場'` 在找的 `"滑雪場"`，也不是測試版常數 `RESORT_KEY_TEST = '(測試) 滑雪場'`。`isFuranoItem()` 讀 `item.properties['滑雪場'] || item.properties['(測試) 滑雪場']`，兩個 key 都讀不到，結果永遠是 `undefined === '富良野'` → `false`，**裝備/住宿卡片因此永遠不會顯示，不管客人選哪個雪場，這是全面性失效，不是偶發**。

對照 PROGRESS.md 2026-08-29 那次的原始查證記錄（見上方「順序6 方向性修正」章節 Step 1 表格）：當時查到的正式環境 key 是 `滑雪場`，選項是「札幌手稻／旭川神居／富良野／佐幌」。**今天實測同一個欄位，key 變成 `雪場區域`，選項也變成「札幌手稻／旭川／富良野／十勝」**（旭川神居→旭川、佐幌→十勝）——選項文字都變了，證實這是 **BTA 後台這個 Booking Field 本身被重新設定過**（欄位標籤改名＋選項文字微調），不是我們這邊程式碼或資料的問題。

用 `test-course-halfday-peak` 額外核對一次（只走到「填寫詳細資料」表單、沒有真的送出），確認同一個欄位一樣是 `雪場區域`——這是 widget 124456 層級共用的欄位設定，**判斷會影響全部 4 個星號正式商品（全天/半天 × 旺季/淡季），不是單一商品的個案**。

### 查證結果彙整（回答 Eric 提出的四點）

1. **真實流程確認**：✅ 確認重現——`test-course-fullday-peak`，Stage2 選「富良野」，送出後 Stage3 只看到「指定教練」卡片，**「官方專屬裝備加租」「專屬特約民宿加購」兩張卡片完全沒有出現**，跟 Eric 回報的症狀一致。
2. **`isFuranoItem()` 有沒有被今天的改動動到**：❌ 沒有。`git show`／`git log -S` 雙重確認，今天 `ea85d1a` 完全沒碰這段邏輯，最後一次改動是 8/29。
3. **Console 有沒有錯誤中斷 Stage3 渲染**：❌ 沒有。全程 Console 檢查，唯一的錯誤是已知的 BTA 幽靈 widget 舊 bug（`bta-widgets-bootstrap.min.js` 的 `querySelectorAll` on undefined）跟無關的 CSS preload 警告，**Stage3 Modal 本身正常渲染、沒有中斷**，只是判斷結果錯誤地把「富良野」也當成「非富良野」處理，不是渲染流程壞掉。
4. **測試商品是哪一個/測試環境還是正式環境**：Eric 沒有明說是哪個商品，這次用 `test-course-fullday-peak`（星號正式商品，widget 124456，網站目前真正在用的那組）重現，並用 `test-course-halfday-peak` 額外核對欄位設定一致——**這是正式環境（客人現在會遇到的），影響全部 4 個星號商品，不是測試環境限定的問題**。

### 結論

**不是今天 `ea85d1a` 這批修復造成的回歸**——今天的改動完全沒有碰到這段判斷邏輯。真正原因是 **BTA 後台的「雪場」Booking Field 在 2026-08-29 順序6完成驗證之後，被重新設定過**（欄位名稱從「滑雪場」改成「雪場區域」，選項文字也有調整），導致程式碼裡寫死比對的 `滑雪場` / `(測試) 滑雪場` 這兩個字串再也對不上，`isFuranoItem()` 從此全面失效。**這是嚴重問題（4 個正式商品、只要客人選富良野，裝備/住宿加購全部消失），但根因跟今天的 commit 無關，是 BTA 後台欄位被改名造成的獨立事件。**

**今晚只查證根因，尚未修改任何程式碼。** 明天可以討論修法方向，初步看到兩個可能：(a) 把 `RESORT_KEY_PROD` 改成 `'雪場區域'`（最直接，但如果 BTA 後台之後又被改名會再壞一次）；(b) 改成不依賴精確欄位名稱比對、改用其他更穩定的判斷依據。兩個方向的可行性跟風險，留到明天再一起評估、確認後才動手。

### 追加：Eric 確認觸發源——BTA 後台欄位「測試」字首清理，範圍比預期更大，至少牽連 2 個欄位

Eric 說明：先前業主反映 BTA 表單每個欄位前面都有「測試」兩字，Eric 自行進 BTA 後台把「測試」字首拿掉。比對今天實測資料跟程式碼常數字串，結果分兩種情況：

- **「同行人員有無兒童」欄位（`hasChildCompanion()`）：跟 Eric 描述的操作完全吻合，確認也被連帶弄壞**。程式碼比對的測試版 key 是 `(測試) 是否有 6~12 歲兒童？（每組最多接受 1 位兒童同行)`（半形符號），正式版 key 是 `是否有 6～12 歲兒童？（每組最多接受 1 位兒童同行）`（全形符號）。今天實測到的真實資料是 `是否有 6~12 歲兒童？（每組最多接受 1 位兒童同行)`——**沒有「(測試) 」字首，但符號維持半形**，剛好就是「只刪字首、其他文字沒動」的結果，兩個 key 都對不上，**這是這次追加查證才發現的第二個受影響欄位**，之前的富良野排查沒有涵蓋到它，會影響 Stage3「同行人員中有兒童」提示是否顯示。
- **「雪場」欄位：Eric 描述的操作只能部分解釋，還有其他異動**。若只是刪掉「(測試) 」字首，「(測試) 滑雪場」應該變回「滑雪場」——這剛好是程式碼已經在比對的正式版字串，理論上不會壞。但今天實測到的欄位名稱是完全不同的「雪場區域」，選項文字也從「札幌手稻／旭川神居／富良野／佐幌」變成「札幌手稻／旭川／富良野／十勝」——**代表這個欄位除了拿掉字首之外，欄位名稱本身跟部分選項文字也被重新編輯過**，不是單純的字首刪除。

**待辦（明天處理）**：修法時至少要同時處理 `isFuranoItem()`（雪場）跟 `hasChildCompanion()`（兒童同行）兩個欄位，不能只修第一個；並建議請 Eric 到 BTA 後台快速核對這次「拿掉測試字首」的清理，還有沒有動到其他 Booking Field 的名稱（例如 `實際參加人數`、`雪板類型`、`課程分級`、`通訊軟體`、`帳號／ID`、`語言`、保險確認欄位等），避免還有第三個沒被發現的欄位判斷失效——今天的驗證裡這些欄位剛好都沒有依賴精確字串比對（`實際參加人數` 只有單一常數、其餘欄位程式碼沒有依賴其 key 做顯示判斷），暫時沒發現受影響，但欄位標籤如果被改，仍可能影響其他未來要新增的邏輯，值得請 Eric 一併確認一次現況。

**Eric 補充澄清（雪場選項文字變動的來源）**：選項文字「旭川神居→旭川、佐幌→十勝」這部分**不是**這次拿掉「測試」字首的操作造成的，是業主舊版欄位本來就掛著過時的區域名稱（旭川神居、佐幌），Eric 這次一併修正成正確的名稱（旭川、十勝）——是獨立於字首清理的另一件事，只是剛好同時發生。也就是「雪場」欄位這次其實疊了兩件事：(1) 拿掉「測試」字首、(2) 順便修正舊版過時的選項文字。這不影響上面的技術結論（無論成因是哪一件，實際寫進購物車的 key 確實變成「雪場區域」，程式碼比對不到），但把完整因果關係補齊，供明天討論修法時參考。
...
**16. `/collections/all` 商品分組視覺優化（低優先，未來有餘力再回顧，2026-09-07 評估後決定暫緩）**

背景：`/collections/all`（購物車頁「View all」連過去的頁面）目前是 11 個乾淨商品的單一清單（4 個星號課程＋6 個真實裝備＋教練加購），沒有依「滑雪課程」/「裝備租賃」分組。Eric 確認導覽選單已有明確分類入口，這頁定位是給「隨便逛逛」的客人看，暫時不投入分組視覺優化。

已查證、評估過的兩個方案，之後要回頭做時可以直接參考，不用重新查證：

- **選項 D（零成本，有隱憂）**：課程商品價格全部 ≥$7,650、裝備商品全部 ≤$1,200，價格區間不重疊，把 collection 排序設成「價格高到低」即可讓兩組自然分開，不需要任何程式碼或後台 tag 操作。**缺點**：沒有「滑雪課程」/「裝備租賃」文字分隔標題；且依賴「課程永遠比裝備貴」這個價格假設，未來如果新增一個高價裝備或低價課程，分組可能失效。
- **選項 E（效果完整，工作量較高）**：Shopify 原生 collection 排序沒有「依標籤排序」選項，且原生 `main-collection` 區塊架構本身不支援在商品清單中間插入文字——要做出「兩組商品中間有文字分隔線」的效果，必須另外寫一個專屬 section 檔案（不能沿用共用的 `main-collection.liquid`），用 Liquid 依 tag（課程統一有 `test-course`、裝備統一有 `gear-rental`，這兩個 tag 本身已經夠乾淨可以直接用）分組渲染，還要處理無限捲動分頁在「兩組分開渲染」情況下的相容性，工作量比原本預期的高。

**待辦**：之後有餘力要做的話，先跟 Eric 確認要選 D 還是 E（或屆時有沒有更好的第三方案），不要預設哪一個。
