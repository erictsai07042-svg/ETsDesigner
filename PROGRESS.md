...
以下 9 組欄位目前都是「正式版／測試版各自獨立、互不牽動」的結構，共用一套判斷原則：**正式版 Label 永遠不帶前綴，Apply tags 只有 `halfday`／`fullday`；測試版 Label 永遠帶「(測試) 」前綴，Apply tags 只有 `test-halfday`／`test-fullday`**（新命名慣例，不再使用舊的 `test-course`）。往後如果只是要調整測試階段的內容，只需要編輯「(測試) XXX」那一份，正式版保證不受影響。
...
**另外兩個不在這張表裡的欄位**（不是「正式/測試分開」的結構，維持原樣）：
...
- 沒有其他例外，BTA 後台目前 Booking Fields 清單共 19 筆（9 組 × 2 + 實際參加人數 1 筆）。
...
### 🔴 下一個對話串優先任務（2026-08-16 新增，接手第一件事先看這兩項）
...
2. **【目前唯一剩餘待辦】課程分級資訊欄位（零基礎/初階，不影響價格）**：新增一個讓客人選擇自己滑雪程度（例如零基礎／初階）的 BTA Booking Field 下拉選單，寫入 line item property，不影響 Shopify Variant/價格，純資訊用途給教練備課參考，不像「人數方案」那樣需要另外疊加前端驗證比對。**比照「滑雪場」欄位拆分模式建立**（見文件最上方對照表）：正式版與測試版從一開始就建成兩個獨立欄位，不要先建一個共用的再事後拆分。正式版 Label 不帶前綴、Apply tags 只設 `halfday`/`fullday`；測試版 Label 加「(測試) 」前綴、Apply tags 只設 `test-halfday`/`test-fullday`（新命名慣例，不要用舊的 `test-course`）。選項內容（零基礎／初階等，確切文字待跟業主確認，不要自行假設）兩邊必須逐字一致。
