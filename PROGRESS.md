...
## ✅ 2026-08-17：指定教練加購選項（NT$400，整組課程共用一位）已完成並驗證通過
...
近期（2026-08-12～08-17）完成：實際參加人數驗證即時提示、`proxyBaseUrl` 問題結案、Redirect 設定調整、Stage 3 加購金額總計、方案資訊內容修正、多天預訂可行性查證（結論：暫不建議，維持單日選擇）、Stage 3 Modal 三項體驗優化（條件式風險聲明／頂部固定顯示／桌機加寬）、課程介紹頁「立即預訂」連結修正、`test-course-halfday-offpeak` 端對端驗證補完、**指定教練加購選項（整組共用一位，NT$400）已完成並驗證通過**（見最上方章節）。**四個測試商品（全日/半日 × 旺季/淡季）的端對端流程全數驗證通過。** 下方「🔴 待辦事項」還剩「課程分級資訊欄位」「Booking Field 分類前綴命名」兩項待辦。
...
3. **Booking Field 分類前綴命名方案，待業主決定 → 待 Claude Code 執行**：2026-08-16 已完成查證（見文件最上方「查證——BTA Booking Field 名稱的程式碼依賴性」專章），結論是**只有「實際參加人數」不可直接改名**（4 處硬編碼，修改難度低但需跟 BTA 後台改名動作同步進行），其餘欄位都安全可改。**業主決定實際要採用的新命名方案（例如確切的分類前綴文字）後，下次接手直接照查證報告裡列的檔案/行數同步修改即可，不需要重新查證一次。**
