...
近期（2026-08-12～08-16）完成：實際參加人數驗證即時提示、`proxyBaseUrl` 問題結案、Redirect 設定調整、Stage 3 加購金額總計、方案資訊內容修正、多天預訂可行性查證（結論：暫不建議，維持單日選擇）、Stage 3 Modal 三項體驗優化（條件式風險聲明／頂部固定顯示／桌機加寬）、課程介紹頁「立即預訂」連結修正、`test-course-halfday-offpeak` 端對端驗證補完（見下方兩節）。**四個測試商品（全日/半日 × 旺季/淡季）的端對端流程現在全數驗證通過。** 下方「🔴 待辦事項」最上方列了下一步兩個優先任務，其中第一項有設計問題需要跟業主確認，不要自行假設。
...
1. **指定教練加購選項（NT$400，三位教練）**——**⚠️ 有一個設計問題還沒確認，新對話串接手時務必先跟業主確認，不要自行假設**：指定教練這個加購，應該是「每位學員各自選一位教練」，還是「整組課程共用同一位教練」？這會直接影響資料結構怎麼設計（比照現有 `學員N_加購_XXX` 每人獨立一筆，還是比照「實際參加人數」這種整筆課程層級的單一欄位）、Stage 3 Modal 要不要在每個學員分組底下都放一次選擇器，或是在最外層放一次就好。**先問清楚再動手，不要套用現有裝備加購的「每學員各自選」模式，因為教練是人力资源、跟裝備的性質不一樣，選錯資料結構之後要改會牽動 Stage3 Modal 結構跟金額計算邏輯。**
