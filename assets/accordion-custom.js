...
document.addEventListener('click', function (event) {
  const toggle = event.target.closest('details > summary');
  if (!toggle) return;

  const currentDetails = toggle.closest('details');
  const parentSection = currentDetails.closest('.accordion');
  if (!parentSection) return;

  parentSection.querySelectorAll('details').forEach(function (detail) {
